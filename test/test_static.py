...
def test_module_path(modspec, ident, path):
    with tutils.tdir():
        root = pdoc.doc.extract_module(modspec)
...
def test_path_to_module():
    with tutils.tdir():
        root = pdoc.doc.extract_module("./modules/submods")
...
def test_static(tmpdir):
    ...
    with tutils.tdir():
        one = pdoc.doc.extract_module("./modules/one")
        two = pdoc.doc.extract_module("./modules/submods")
